...
export default function ShopView({
  ...
  // --- Filtering Logic ---
  const getFilteredShopItems = useMemo(() => {
    ...
    let filtered = shopItems.filter(item => {
      ...
      // Assuming is_gacha_exclusive might be on the item object from DB
      const isGachaOnly = (item as any).is_gacha_exclusive === true; 
      return !isOwned && !isGachaOnly;
    });

    if (activeShopTab === 'all') {
      // Sort featured logic if available, otherwise default sort
      return filtered;
    }
    ...
    switch (activeShopTab) {
      ...
      case 'accessories': return filtered.filter(item => ['accessory', 'jewelry', 'charms', 'scarves', 'earrings'].includes(item.slot));
  ...
  }, [shopItems, user.cosmetics, activeShopTab]);
        ...
        <View style={styles.priceRow}>
          {item.price > 0 && (
            <View style={styles.priceTag}>
              <Image source={coinIcon} style={styles.priceIcon} />
              <Text style={styles.priceText}>{item.price}</Text>
            </View>
          )}
          {(item as any).gem_price > 0 && (
            <View style={styles.priceTag}>
              <Image source={gemIcon} style={styles.priceIcon} />
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.mainTabs}>
        ...
        >
          <Image source={weaponsIcon} style={styles.mainTabIcon} />
        ...
        >
          <Image source={cosmeticsIcon} style={styles.mainTabIcon} />
        ...
        >
          <Image source={gachaIcon} style={styles.mainTabIcon} />
      ...
      <View style={styles.currencyHeader}>
        ...
        <View style={styles.currencyContainer}>
          ...
          <View style={styles.currencyBox}>
            <Image source={gemIcon} style={styles.currencyIcon} />
      ...
      {activeMainTab === 'magic' && (
        <FlatList
          data={getFilteredShopItems.filter(i => i.slot === 'magic effects')}
          keyExtractor={item => item.id}
          numColumns={2}
          contentContainerStyle={styles.gridContainer}
          renderItem={renderItemCard}

...
const RankingsPanel: React.FC<RankingsPanelProps> = ({ 
  ...
  const renderPlayer = ({ item, index }: { item: any, index: number }) => {
    ...
    return (
      <View style={[styles.playerCard, getRankingClass(index)]}>
        <View style={styles.playerLeft}>
          ...
          <View style={styles.playerInfo}>
            ...
            <Text style={styles.playerSubtitle}>
              {item.current_title || 'Hunter'} • LV. {item.level}
            </Text>
          </View>
        </View>
        <View style={styles.playerRight}>
          <Text style={styles.cpLabel}>COMBAT_POWER</Text>
          <Text style={styles.cpValue}>{item.showcase_score || 0}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>ELITE_HUNTER_RANKINGS</Text>

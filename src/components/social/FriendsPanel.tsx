    ...
    >
      ...
      {!query && suggestions.length > 0 && (
        <View style={styles.section}>
          {renderSectionHeader('DETECTED_HIGH_RESONANCE_SIGNALS')}
          <View style={styles.suggestionsGrid}>
            {suggestions.map((hunter) => (
              <View key={hunter.id} style={styles.suggestionCard}>
                ...
                <TouchableOpacity 
                  onPress={() => handleAddFriend(hunter.id)}
                  style={styles.actionButtonSmall}
                >
                  <Text style={styles.actionButtonTextSmall}>INITIATE_SYNC</Text>
                </TouchableOpacity>
              </View>
            ))}
          </View>
            ...
            <View key={request.id} style={styles.requestCard}>
              <View style={styles.requestLeft}>
                <View style={styles.avatarMiniWrapper}>
                  <Image 
                    source={request.requester?.avatar ? { uri: request.requester.avatar } : require('../../../assets/huntericon.png')} 
      ...
      {query && searchResults.length > 0 && (
        <View style={styles.section}>
          {renderSectionHeader('SEARCH_RESULTS', '#4ade80')}
          {searchResults.map((hunter) => (
            <View key={hunter.id} style={styles.requestCard}>
              <View style={styles.requestLeft}>
                <LayeredAvatar user={hunter} size={40} onAvatarClick={() => setSelectedAvatar(hunter)} />
              ...
              <TouchableOpacity 
                onPress={() => handleAddFriend(hunter.id)}
                style={[styles.miniButton, styles.acceptButton, { paddingHorizontal: 12 }]}
              >
                <Text style={styles.actionButtonTextSmall}>ADD_SYNC</Text>
              </TouchableOpacity>
            </View>
          ))}
        </View>
      ...
      {/* Friends Grid */}
      <View style={styles.section}>
        {renderSectionHeader('HUNTER_NETWORK')}
        {friends.length === 0 ? (
          <View style={styles.emptyFriends}>
            <Text style={styles.emptyText}>NO_FRIENDS_YET</Text>
            ...
            {friends.map((friend) => (
              ...
              >
                <View style={styles.friendAvatarWrapper}>
                  <LayeredAvatar user={friend} size={56} onAvatarClick={() => setSelectedAvatar(friend)} />

...
function getBattleFieldsFromRow(row: Record<string, unknown> | null | undefined) {
  if (!row) return { name: '', cost: 1, basePower: 50, type: 'damage', element: 'Physical', cooldown: 0, description: '' };
  ...
  return {
    ...
    type: rawType === 'physical' || rawType === 'magic' ? 'damage' : rawType,

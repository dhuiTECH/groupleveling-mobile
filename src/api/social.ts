...
export const api = {
  ...
  voteShowcase: async (voterId: string, targetId: string, voteType: 'resonate' | 'interfere') => {
    ...
    const { data: existing } = await supabase
      ...
      .eq('vote_month', voteWeek)
      .limit(1);
    if (existing && existing.length > 0) throw new Error('You have already voted this week.');
    ...
    // Update target's showcase_score
    const { data: targetProfile, error: targetErr } = await supabase
      .from('profiles')
      .select('showcase_score')
      .eq('id', targetId)
      .single();
    if (!targetErr && targetProfile) {
      const newScore = (Number(targetProfile.showcase_score) || 0) + value;
      await supabase.from('profiles').update({ showcase_score: newScore }).eq('id', targetId);
    }

    // Award 250 coins to voter
    const { data: voterProfile, error: voterErr } = await supabase
      .from('profiles')
      .select('coins')
      .eq('id', voterId)
      .single();
    if (!voterErr && voterProfile) {
      const newCoins = (Number(voterProfile.coins) || 0) + 250;
      await supabase.from('profiles').update({ coins: newCoins }).eq('id', voterId);
      return { newCoins };
    }
    return {};
  },

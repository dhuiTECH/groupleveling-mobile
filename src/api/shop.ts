  ...
  purchaseItem: async (
    ...
    try {
      ...
      if (currency === 'both') {
        ...
        if (currentGems < (shopItem.gem_price || 0)) throw new Error('Insufficient gems');
      } else {
        if (currentCoins < (shopItem.price || 0)) throw new Error('Insufficient coins');
      }
      ...
      if (currency === 'both') {
        ...
        updateData = { gems: currentGems - (shopItem.gem_price || 0) };
        newGemBalance = updateData.gems;
      } else {
        updateData = { coins: currentCoins - (shopItem.price || 0) };
        newCoinBalance = updateData.coins;
      }
      ...
      const { data: newCosmetic, error: insertError } = await supabase
        ...
        .select(`
          id,
          equipped,
          created_at, // mapped from acquired_at in API but assuming standard columns
          shop_item_id,
      ...
      return {
        ...
        newBalance: currency === 'gems' ? newGemBalance : newCoinBalance,
        newCoinBalance,
        newGemBalance
      };
